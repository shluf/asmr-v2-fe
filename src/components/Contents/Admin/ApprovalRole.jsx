    ...
    const handleApprovalAction = async (pendingId, newStatus) => {
        ...
        try {
            ...
            if (
                ...
                selectedWargaPending.user.id === pendingId
            ) {
        ...
        <div className="w-full p-6">
                            ...
                            {isLoading ? (
                                ...
                                <TableRow>
                                    <TableCell
                                        ...
                                        Tidak ada permintaan persetujuan akun
                                        saat ini.
                                    </TableCell>
                                </TableRow>
                            ) : (
            ...
            {selectedWargaPending && (
                <Dialog
                    ...
                    onOpenChange={isOpen => {
                        if (!isOpen) setSelectedWargaPending(null)
                    ...
                    <DialogContent className="sm:max-w-lg">
                        <DialogHeader>
                            <DialogTitle>
                                ...
                                {selectedWargaPending.nama}
                            </DialogTitle>
                            <DialogDescription>
                                Tinjau detail data warga yang mengajukan
                                persetujuan akun.
                            </DialogDescription>
                        </DialogHeader>
                        <div className="grid gap-y-2 gap-x-4 py-4 text-sm max-h-[60vh] overflow-y-auto pr-2">
                            <DataField
                                ...
                                label="Nama Lengkap"
                                value={selectedWargaPending.nama || 'N/A'}
                            />
                            <DataField
                                label="Email"
                                value={selectedWargaPending.user.email || 'N/A'}
                            />
                            <DataField
                                className="md:col-span-2"
                                label="Alamat"
                            ...
                            <DataField
                                ...
                                value={`${selectedWargaPending.tempat_lahir || 'N/A'}, ${formatDateSafe(selectedWargaPending.tanggal_lahir)}`}
                            />
                            <DataField
                                label="Agama"
                                value={selectedWargaPending.agama || 'N/A'}
                            />
                            <DataField
                                label="NIK"
                                value={selectedWargaPending.nik || 'N/A'}
                            />
                            <DataField
                                label="No. KK"
                                value={selectedWargaPending.nomor_kk || 'N/A'}
                            />
                            <DataField
                                label="Jenis Kelamin"
                                value={
                                    selectedWargaPending.jenis_kelamin ===
                                    'Pria'
                                        ? 'Laki-laki'
                                        : selectedWargaPending.jenis_kelamin ===
                                            'Perempuan'
                            ...
                            <DataField
                                label="No. Telepon"
                                value={selectedWargaPending.phone || 'N/A'}
                            />
                            <DataField
                                label="RT"
                                value={
                                    selectedWargaPending.rt?.nama_rt || 'N/A'
                                }
                            />
                            <DataField
                                label="RW"
                                value={
                                    selectedWargaPending.rt?.rw?.nama_rw ||
                                    'N/A'
                                }
                            />
                            <DataField
                                ...
                                label="Status Akun"
                                value={
                                    selectedWargaPending.user.status_akun == 1
                                        ? 'Disetujui'
                                        : selectedWargaPending.user
                                                .status_akun == 2
                                          ? 'Ditolak'
                                          : 'Pending'
                                }
                            />
                            {selectedWargaPending.catatan && (
                                <DataField
                                    className="md:col-span-2"
                                    label="Catatan Sebelumnya"
                                    value={selectedWargaPending.catatan}
                                />
                            )}
                        </div>
